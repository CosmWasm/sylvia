...
mod tests {
    ...
    fn dispatch_replies() {
        ...
        let err = contract
            .send_message_expecting_data(Some(Binary::default()), DATA_OPT_REPLY_ID)
            .call(&owner)
            .unwrap_err();
        assert_eq!(
            err,
            StdError::generic_err("Invalid reply data: \nSerde error while deserializing Error parsing into type alloc::string::String: EOF while parsing a JSON value.").into()
        );
        ...
        let err = contract
            .send_message_expecting_data(Some(Binary::default()), DATA_REPLY_ID)
            .call(&owner)
            .unwrap_err();
        assert_eq!(
            err,
