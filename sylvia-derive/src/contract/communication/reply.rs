    ...
    fn emit_data_deserialization(&self) -> TokenStream {
        ...
        match data {
            ...
            Some(data) if data.opt => quote! {
                let data = match data {
                    Some(data) =>
                        Some( #sylvia ::cw_std::from_json(&data).map_err(|err| #sylvia ::cw_std::StdError::generic_err( #invalid_reply_data_err ))?),
                    None => None,
                };
            },
            None if is_data_attr => quote! {
                let data = match data {
                    Some(data) =>
                        #sylvia ::cw_std::from_json(&data).map_err(|err| #sylvia ::cw_std::StdError::generic_err( #invalid_reply_data_err ))?,
                    None => return Err(Into::into( #sylvia ::cw_std::StdError::generic_err(#missing_data_err))),
                };
            },
            _ => {
