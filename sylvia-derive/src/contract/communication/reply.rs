    ...
    fn emit_data_deserialization(&self) -> TokenStream {
        ...
        match data {
            Some(data) if data.raw && data.opt => quote! {
                if data.is_none() {
                    return Err( #sylvia ::cw_std::StdError::generic_err(#missing_data_err)).map_err(Into::into);
                }
            },
            Some(data) if data.raw => quote! {
                let data = match data {
                    Some(data) => data,
                    None => return Err( #sylvia ::cw_std::StdError::generic_err(#missing_data_err)).map_err(Into::into);
                };
            },
            Some(data) if data.opt => quote! {
                let data = match data {
                    Some(data) =>
                        Some(#sylvia ::cw_std::from_json(&data).map_err(|err| Err( #sylvia ::cw_std::StdError::generic_err(format! {"Invalid reply data: {}\nSerde error while deserializing {}", data, err})).map_err(Into::into))?),
                    None => None;
                };
            },
            Some(_) => quote! {
                let data = match data {
                    Some(data) =>
                        #sylvia ::cw_std::from_json(&data).map_err(|err| Err( #sylvia ::cw_std::StdError::generic_err( #invalid_reply_data_err )).map_err(Into::into))?,
                    None => return Err( #sylvia ::cw_std::StdError::generic_err(#missing_data_err)).map_err(Into::into);
                };
            },
            None => {
