    ...
    fn as_reply_data(&self) -> Vec<ReplyData> {
        ...
        self
            ...
            .for_each(|(reply_id, function_name, reply_on)| {
                match reply_data.iter_mut().find(|existing_data| existing_data.reply_id == reply_id) {
                    Some(existing_data)
                        if existing_data.handlers
                            .iter()
                            .any(|(_, prev_reply_on)| {
                                prev_reply_on.excludes(&reply_on)
                            }) =>
                    {
                        existing_data.handlers.iter().for_each(|(prev_function_name, prev_reply_on)|{
                            emit_error!(reply_id.span(), "Duplicated reply handler.";
                                note = prev_function_name.span() => format!("Previous definition of handler={} for reply_on={}", existing_data.reply_id.to_string(), prev_reply_on)
                            )
                        })
                    },
                    Some(existing_data) => {
                        existing_data.handlers.push((function_name, reply_on))
                    },
...
impl<'a> ReplyData<'a> {
    ...
    fn emit_match_arms(&self, contract: &Type, generics: &[&GenericParam]) -> TokenStream {
        ...
        let success_match_arm = match handlers
            .iter()
        ...
        let failure_match_arm = match handlers
            .iter()
