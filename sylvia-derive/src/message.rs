    ...
    pub fn emit_dispatch_leg(&self, msg_type: MsgType) -> TokenStream {
        ...
        match msg_type {
            ...
            Instantiate | Migrate | Reply | Sudo => {
                emit_error!(name.span(), "Instantiation, Reply, Sudo and Migrate messages not supported on traits, they should be defined on contracts directly");
                quote! {}
            }
        }
    }
...
impl<'a> EntryPoints<'a> {
    ...
    pub fn emit(&self) -> TokenStream {
        ...
        #[cfg(not(tarpaulin_include))]
        {
            let instantiate_ep = override_entry_points
                .get_entry_point(MsgType::Instantiate)
                .map(|ep| ep.emit_entry_point(&custom_msg, error))
                .unwrap_or_else(|| {
            ...
            let exec_ep = override_entry_points
            ...
            let query_ep = override_entry_points
